...
sugiyama.partitioner = function(graph){
	...
	var num_parts = 0;
	each(vertex_partition_set, function(key){
	    num_parts++;
	});
	...
	num_parts = 0;
	each(edge_partition_set, function(key){
	    num_parts++;
	});
	(function(){
	    for( var i = 0; i < num_parts; i++ ){
	...
	var i = 0;
	each(edge_partition_set, function(key){ i++; });
	return i;
    };
...
var cytoscape_layout_engines = function(ngraph, layout){
...
var layout = function(){
    ...
    anchor.render = function(graph, engine, args){
